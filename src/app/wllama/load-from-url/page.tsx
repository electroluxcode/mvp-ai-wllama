...
export default function ModelLoader() {
  ...
  const loadFromRemote = async () => {
    if (!remoteUrl.trim() || !wllamaCoreRef.current) return;
    setLoading(true);
    setError('');
    try {
      await wllamaCoreRef.current.loadModelFromUrl(remoteUrl, { n_ctx: 4096, n_batch: 128 });
      const metadata = wllamaCoreRef.current.getModelMetadata();
      setModelName(metadata?.name || remoteUrl.split('/').pop()?.replace('.gguf', '') || '远程模型');
        ...
        <div className="max-w-3xl mx-auto space-y-3">
          <div className="flex gap-2">
            <input
              ...
              placeholder="输入模型URL..."
              defaultValue={"https://huggingface.co/LiquidAI/LFM2-700M-GGUF/resolve/main/LFM2-700M-Q4_K_M.gguf"}
              value={remoteUrl}
              onChange={(e) => setRemoteUrl(e.target.value)}
              disabled={loading}
              onKeyDown={(e) => e.key === 'Enter' && loadFromRemote()}
            />
            <button
